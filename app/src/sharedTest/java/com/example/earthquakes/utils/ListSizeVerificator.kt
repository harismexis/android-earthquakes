...
fun <T> verifyListSizeWhenAllIdsValid(items: List<T>) {
    verifyListSize(BaseMockParser.EXPECTED_NUM_QUAKES_WHEN_ALL_IDS_VALID, items)
}

fun <T> verifyListSizeWhenSomeIdsAbsent(items: List<T>) {
    verifyListSize(BaseMockParser.EXPECTED_NUM_QUAKES_WHEN_TWO_IDS_ABSENT, items)
}

fun <T> verifyListSizeWhenSomeItemsEmpty(items: List<T>) {
    verifyListSize(BaseMockParser.EXPECTED_NUM_QUAKES_WHEN_TWO_EMPTY, items)
}

fun <T> verifyListSizeForNoData(items: List<T>) {
    verifyListSize(BaseMockParser.EXPECTED_NUM_QUAKES_WHEN_NO_DATA, items)
}